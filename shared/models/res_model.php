...
class Res_model extends MY_Model {
    ...
    public function __construct() {
        ...
        $this->load->model('user_res_model');
    }
    
    public function get_all_by_uid($uid=0){
        $options = array('user_res.uid'=>$uid);
        
        $options['select'] = array("$this->_table.*");
         
        $join[] 		    = array("user_res" ,"user_res.rid=$this->_table.id");
        $options['join'] = $join;
    ...
    public function res_exist($hash='',$uid=''){
        ...
        if($item && !$this->user_res_model->getOne(array('uid'=>$uid,'rid'=>$item->id))){
            $this->user_res_model->add(array('uid'=>$uid,'rid'=>$item->id));
        }
    ...
    public function delete(Array $options=array()){
        ...
        if(isset($options['uid'])){
            ...
            $this->user_res_model->delete($options2);
            
            $options2['group_by'] = 'rid';
            unset($options2['uid']);
            
            foreach ($this->user_res_model->getAll2Array($options2) as $ur){
    ...
    public function add(Array $data=array()){
        ...
        $this->db->trans_start();
            ...
            if(isset($data['user_id'])){
                $this->user_res_model->add(array('user_id'=>$data['user_id'],'rid'=>$rid));
            }
    	//关闭事务
    	$this->db->trans_complete();
        return $rid;

...
class UploadHandler
{
    ...
    protected function set_additional_file_properties($file) {
        $file->deleteUrl = $this->options['script_url']
            ...
            ."[$file->id]=".rawurlencode($file->name);
     ...
     */
    protected function get_file_object(Array $res=array()) {
        $file_name = $this->get_hash_name($res['id']).$res['ext'];
...
//         )));
        ...
        $file_list = $this->ci->res_model->get_all_by_uid($this->ci->user_info->id);
//         print_r($file_list);
...
//             print_r($file_list);die;
     ...
     * @param stdClass $file
     */
    protected function save_res($hash='',&$file){

        $suffix = '.'.end(explode(".", $file->name));
        
        $res = $this->ci->res_model->res_exist($hash,$this->ci->user_info->id);
        if(!$res){
            $data = array(
                            'hash'  =>$hash,
                            'ext'   =>$suffix,
                            'type'  =>$file->type,
                            'size'  =>$file->size,
                            'user_id'   =>$this->ci->user_info->id
                        );
            $res = new stdClass();
            $res->id = $this->ci->res_model->add($data);
        }
        $file->id = $res->id;
        $file->name = $this->get_hash_name($file->id).$suffix;
    }
    
    public function get_hash_name($id=0){
        return md5($id.self::HASH_CODE);
    }
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
        ...
        $file->type = $type;
        //资源保存数据库，并根据资源id生成文件名
        $this->save_res(md5_file($uploaded_file),$file);
...
//         }
        ...
        $options = array('id'=>array_keys($file_names),'uid'=>$this->ci->user_info->id);
        
        $ids = $this->ci->res_model->delete($options);
        foreach($file_names as $id=>$file_name) {

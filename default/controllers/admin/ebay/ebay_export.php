...
class Ebay_export extends Admin_Controller
{
    ...
    private $ebay_paypal;
    private $site_rate;
    private $site_distinguish;

    function __construct()
    {
        $this->site_distinguish = array(
            0=>array(
                'pp'=>10,
                1=>20,
                2=>5 ),
        ...
        $this->site_rate = array(3=>0.68);
        $this->platfrom =array(0=>14,3=>12);
        $this->ebay_shipment =array(
            ...
            3=>array(
                1=>'UK_EconomyShippingFromOutside',
                2=>'UK_SellersStandardInternationalRate',
                3=>'UK_OtherCourierOrDeliveryInternational',
            )
        );
        ...
        if (isset($_REQUEST['add']) && ($_REQUEST['add'] == 'add')) {
            ...
            for ($j = 2; $j <= $rows; $j++) {
                if (trim($sheet->getCell("A" . $j)->getValue())) {
                    ...
                    $data_array[$i]['item_status'] = trim($sheet->getCell("T" . $j)->getValue());//物品状态
                    $i++;
                }
            }
            ...
            foreach ($data_array as $list) {
                ...
                if(isset($this->site_rate[$list['site']])){ //
                    $price = $price/$this->site_rate[$list['site']];
                }

                if($price <$this->site_distinguish[$list['site']]['pp']){
                ...
                if($price>$this->site_distinguish[$list['site']][1]){
                    ...
                    if($price>$this->site_distinguish[$list['site']][2]){ // 这是要计算出运费
                        $eub_price = $this->calculateSkupriceMid(272,$product_info[0],$list['site']); // eub 价格

                        if(isset($this->site_rate[$list['site']])){ //
                            $eub_price = $eub_price/$this->site_rate[$list['site']];
                        }
                        ...
                        if($list['site']==0){
                            ...
                            $list['international_type1'] =$this->ebay_shipment[$list['site']][2];
                            $list['international_cost1'] =$dif_price;
                            $list['international_extracost1'] =$dif_price;
                            $country1 = array('US');
                            $list['international_is_country1'] =json_encode($country1);


                            $list['international_type2'] =$this->ebay_shipment[$list['site']][3];
                            $list['international_cost2'] =0.00;
                            $list['international_extracost2'] =0.00;
                            $country1 = array('CN','RU','CA','BR','DE','FR','Europe','GB','EuropeanUnion','Asia','AU','MX','JP');
                            $list['international_is_country2'] =json_encode($country1);
                        }
                ...
                if($list['price'] >= $this->ebay_paypal[$list['ebayaccount']][$list['site']][1]){
                    $list['paypal_account'] = $this->ebay_paypal[$list['ebayaccount']][$list['site']][2];
                }else{
                    $list['paypal_account'] = $this->ebay_paypal[$list['ebayaccount']][$list['site']][3];
                }

...
class Ebaylist extends Admin_Controller{
    ...
    public function ebaylistinfo()
    {
        ...
        if (isset($search['productStatusType']) && $search['productStatusType']) {
            $where[$this->ebaylist->_table.'.status'] = trim($search['productStatusType']);
        ...
        if (isset($search['user_id']) && $search['user_id']) {
            $where['ad_user'] = trim($search['user_id']);
        ...
        if (isset($search['creattime1']) && $search['creattime1']) {
            $where['starttime >'] = trim($search['creattime1']);
            $string[]           = 'search[creattime1]=' . trim($search['creattime1']);
        }
        if (isset($search['creattime2']) && $search['creattime2']) {
            $where['starttime <'] = trim($search['creattime2']);
            $string[]           = 'search[creattime2]=' . trim($search['creattime2']);
        }

     //   $select = array("{$this->ebaylist->_table}.*",'e.user_id','e.list_id');
        $select = array("{$this->ebaylist->_table}.*");
      //  $join[] = array($this->Ebay_task_list_model->_table.' e',"e.list_id={$this->ebaylist->_table}.id");

        $search = $search ? $search : array();

        $orderBy = $this->ebaylist->_table.'.id DESC';
        //查询条件
        $options     = array(
            'select' =>$select,
            'where'    => $where,
        ...
        $data_list   = $this->ebaylist->getAll2Array($options,$return_data);
    //    echo $this->db->last_query();echo "<br/>";exit;
// myecho($data_list);
        //add lidabiao 2016-5-4
        if (isset($search['sku']) && $search['sku']) {
           // $like['sku'] = trim($search['sku']);
            $this->db->like('sku_search', $search['sku'], 'after');
        }
     ...
     //  echo $this->db->last_query();exit;
      ...
      /*  $task_user_option = array();
        ...
        $data['all_user'] = $this->ebaylist->defineEbaySellerPrefix(2);
        $data['user_name'] = $this->slme_user_model->get_all_user_info('user_name');

       // myecho($data['all_user']);

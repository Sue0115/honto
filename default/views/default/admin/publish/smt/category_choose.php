	...
	<div class="row row-border">
		<div class="proh">
			<div class="hideaccordion">
				<h1>
					<i class="icon-plus"></i> <span>&nbsp;一般信息</span>
				</h1>
			</div>
		</div>
		...
		<div class="probody">
			<div class="procnt">
			    <div class="form-group">
			        <label class="col-sm-2 control-label">请选择账号：</label>
			        <div class="col-sm-10">
			            <select name="token_id" id="token_id">
			                <?php foreach($token_list as $token):?>
			                <option value="<?php echo $token['token_id'];?>" <?php echo $token_id == $token['token_id'] ? 'selected="selected"' : '';?>><?php echo $token['token_id'].'-'.$token['accountSuffix'];?></option>
			                <?php endforeach;?>
			            </select>
			        </div>
			    </div>
					    ...
					    <div id="category_select">
    						<div class="col-sm-3 no-padding-left">
    							<select size="10" class="form-control category_list" multiple>
    								...
    								<option value="<?php echo $item->category_id;?>" lang="<?php echo $item->isleaf;?>"><?php echo $item->category_name;?></option>
    							    <?php endforeach;?>
    							</select>
    						</div>
						</div>
			...
			<div class="promsg" style="display: none;">类目信息</div>
		</div>
	</div>
</form>
...
$(function(){
	...
	$(document).on('change', '.category_list', function(){
		...
		if (a_isleaf == '0'){
			$.ajax({
				url: '<?php echo admin_base_url('publish/smt/showChildCategory');?>',
	...
	$(document).on('click', '#command_btn', function(){
		...
		if (keyword != '' && token_id != ''){
			...
			$.ajax({
				url: '<?php echo admin_base_url('publish/smt/showCommandCategoryList');?>',
